...
if ($nn>$dailylimit)  {echo "exceeded daily limit";}
else {echo "<br>";echo ($dailylimit-$nn);echo " more stores remaining today<br>";}
}
...
if ($myport==443) {$finalurl="https://".$myhttp;} else {
$finalurl="https://".$myhttp.":".$myport;}
$finalurl2="https:\/\/".$myhttp.":".$myport;
...
$ext=$a;
if (strpos($ext,"?")>0) {$ext=substr($ext,0,strpos($ext,"?"));}
if (strpos($ext,".")>0) {$ext=substr($ext,strrpos($ext,"."));}
...
$b=substr($b,0,10);
if (strpos($b, "image")!==FALSE) {return "img";}
if (strpos($b, "audio")!==FALSE) {return "snd";}
if (strpos($b, "vid")!==FALSE) {return "vid";}
if (strpos($b, "text")!==FALSE) {return "text";}
return "vid";
}
else {return "";}
}
...
if (startsWith($s,"&apos;")) {$s=substr($s,6);}
if (endsWith($s,"&apos;")) {$s=substr($s,0,strlen($s)-6);}
if (startsWith($s,"&quot;")) {$s=substr($s,6);}
if (endsWith($s,"&quot;")) {$s=substr($s,0,strlen($s)-6);}
if (startsWith($s,"&#039;")) {$s=substr($s,6);}
if (endsWith($s,"&#039;")) {$s=substr($s,0,strlen($s)-6);}
...
global $isfacebook;
if ($isfacebook) {$s=str_replace("https\\3a ","https:",$s);
$s=str_replace(" ","",$s);
$s=str_replace("\\3d","=",$s);
$s=str_replace("\\26","&",$s);
...
$size="";
if (strrpos($ff[$i]," ")>0) {$size=substr($ff[$i],strrpos($ff[$i]," "));
}
...
if (strpos($path,"?")>0) {$path=substr($path,0,strpos($path,"?"));
//echo "path:".$path;
}
...
else if (strpos($ee,"+")!==FALSE) {
//echo "javascript caught";
$test=$oldee;$result=false;$index=$index;$zzz[0]=$result;$zzz[1]=$test;$zzz[2]=$index;$zzz[3]=$test;global $scriptopen;$scriptopen=true;return $zzz;}
//else if (strpos($ee," ")!==FALSE) {echo "javascript caught";$test=$oldee;$result=false;$index=$index;$zzz[0]=$result;$zzz[1]=$test;$zzz[2]=$index;$zzz[3]=$test;global $scriptopen;$scriptopen=true;return $zzz;}
...
if (strpos($temp2,"/")>0) {$temp2=substr($temp2,0,strpos($temp2,"/"));}

$baseurlz=$temp.$temp2;
for ($i=0;$i<$adc-1;$i++) {if  (startsWith($a,$adsites1[$i])) {$a="https://0.0.0.0/adblockedorhashflag\n";
if (strpos($adsites1[$i],".wordpress.")>0 || strpos($adsites1[$i],".wp.")>0) {$iswp=true;}}}//endif/if/for
if ($iswp && ($a===$baseurlz."/feed\n" ||$a===$baseurlz."/feed/\n" )) {$a="https://0.0.0.0/adblockedorhashflag\n";
}
else {
//echo "<br>not culprit:".$a;echo " ".$baseurlz; echo " ".($iswp)?"true":"false";
}
if (strpos($a, "vice.com/")>0 && strpos($a, "/static/vendor.ui-components.")>0 ) {$a="https://0.0.0.0/scriptblocked\n";}
if (strpos($a, "medium.com/")>0 && strpos($a, "lite/static/js/vendors~main.")>0 ) {$a="https://0.0.0.0/scriptblocked\n";}

if (strpos($a,"/status/https:\\/\\/abs.twimg.com\\/hashflags\\/")>0 ) {$a="https://0.0.0.0/adblockedorhashflag\n";}
return $a;} 
...
if (strpos($fullpath,"/")>0) {$url=substr($fullpath,0,strpos($fullpath,"/"));
...
if (strToLower($url)==="washingtonpost.com" || strToLower($url)==="www.washingtonpost.com" || strToLower($url)==="www.bloomberg.com"|| strToLower($url)==="bloomberg.com") {
//curl_setopt($ch, CURLOPT_PROXY, "73.46.67.172:58517");
curl_setopt($ch,CURLOPT_HEADER, 1);
curl_setopt($ch,CURLOPT_COOKIE,$cookies);
} else {curl_setopt($ch,CURLOPT_COOKIE,"");
//echo "<br>Not wapo".$url;
}
...
//curl_setopt($ch, CURLOPT_HEADER, 1);
if (strToLower($url)==="twitter.com" || strToLower($url)==="www.twitter.com" || strToLower($url)==="i.imgur.com" || strToLower($url)==="nature.com" || strToLower($url)==="www.nature.com")  {curl_setopt($ch,CURLOPT_HTTPHEADER, array('accept: */*','Accept-Encoding: identity','User-Agent: Wget/1.19.4 (linux-gnu)'));}
else if (strToLower($url)==="www.bloomberg.com" || strToLower($url)==="www.bloomberg.com") {
...
else{if (strpos($result, "Bad Request")<105 && strpos($result, "Bad Request")>0 ) {echo "Bad Request";}
//else {}
if (strToLower($url)==="washingtonpost.com" || strToLower($url)==="www.washingtonpost.com" || strToLower($url)==="www.bloomberg.com"|| strToLower($url)==="bloomberg.com") {
...
foreach($ex as $a) {if (strpos($a,"Set-Cookie:")>0){
//echo "<br>".$a;
$a=substr($a,13);$cookies.=substr($a,0,strpos($a,";"));} else {
//echo "<br>".$a;
}}
//echo "<br><br>Cookies:";
//echo $cookies;
...
$scriptopen=false;
//echo "<br>path:".$path;
if (endsWith($path,".js") || endsWith($path,".json")) {$scriptopen=true;}
...
if ($dd==2) {
if (startsWith(strToLower($cc[1]),"html") || startsWith(strToLower($cc[1]),"head>") || startsWith(strToLower($cc[1]),"!doctype html") ) {
$turl=$url;
if (strpos($turl,"?")) {$turl=strpos($turl,0,strrpos("?"));}
if (strpos($turl,"/")) {$turl=strpos($turl,0,strrpos("/"));}
...
if (endsWith($opath,".js") || endsWith($opath,".json")) {$scriptopen=true;}
...
if (startsWith(strtolower($cc[$dd]),"script") || startsWith(strtolower($cc[$dd]),"img ") || startsWith(strtolower($cc[$dd]),"div ")) {
...
$tmp2=$ee;$ee=substr($ee,0,strpos($ee,"\""));
$tmp3=substr($tmp2,strpos($tmp2,"\"")).$tmp3;
$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); 
if ($kk[0]) {
$urls.=$kk[1];
$urls2[$kk[3]]=1+count($urls2);
$xurls.=(startsWith($cc[$dd],"s")?(($isyoutube && strpos($kk[1],"/base.js")>0)?"vid\n":"src\n"):"img\n");
$index++;
$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".(startsWith($cc[$dd],"s")?"src":"img");} 
else if (startsWith(strToLower($kk[1]),"data:image")) {$tmp2=$kk[1];} 
else if (startsWith(strToLower($kk[1]),"\"data:image")) {$tmp2=$kk[1];} 
else if (strpos($kk[1],"+")!==false) {$tmp2=$kk[1];}
else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".(startsWith($cc[$dd],"s")?"src":"img");}}

else if (strpos(strtolower($ee)," src='")>0)  {$ee=substr($ee,  strpos(strtolower($ee)," src='")+6);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;
$ee=substr($ee,0,strpos($ee,"'"));$tmp3=substr($tmp2,strpos($tmp2,"'")).$tmp3;$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
 if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);
//echo ($isyoutube?"<br>isyoutube":"<br>notyoutube");
//echo "<br>kk[1]".$kk[1];
$xurls.=(startsWith($cc[$dd],"s")?(($isyoutube && strpos($kk[1],"/base.js")>0)?"vid\n":"src\n"):"img\n");
$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".(startsWith($cc[$dd],"s")?"src":"img");} 
else if (startsWith(strToLower($kk[1]),"data:image")) {$tmp2=$kk[1];} 
else if (startsWith(strToLower($kk[1]),"\"data:image")) {$tmp2=$kk[1];} 
else if (strpos($kk[1],"+")!==false) {$tmp2=$kk[1];}
else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".(startsWith($cc[$dd],"s")?"src":"img");}}

else if (strpos(strtolower($ee)," src=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," src=")+5);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);
$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}}
else {$ee=substr($ee,0,strlen($ee)-1);$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}}
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);
$xurls.=(startsWith($cc[$dd],"s")?(($isyoutube && strpos($kk[1],"/base.js")>0)?"vid\n":"src\n"):"img\n");
$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".(startsWith($cc[$dd],"s")?"src":"img");} 
else if (startsWith(strToLower($kk[1]),"data:image")) {$tmp2=$kk[1];} 
else if (startsWith(strToLower($kk[1]),"\"data:image")) {$tmp2=$kk[1];} 
...
if (strpos(strtolower($ee)," data-url=\"")>0) {$ee=substr($ee,  strpos(strtolower($ee)," data-url=\"")+11);
$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;$ee=substr($ee,0,strpos($ee,"\""));$tmp3=substr($tmp2,strpos($tmp2,"\"")).$tmp3;
$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);
$xurls.=(startsWith($cc[$dd],"s")?(($isyoutube && strpos($kk[1],"/base.js")>0)?"vid\n":"src\n"):"img\n");
$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".(startsWith($cc[$dd],"s")?"src":"img");}
...
else if (strpos(strtolower($ee)," data-url='")>0)  {$ee=substr($ee,  strpos(strtolower($ee)," data-url='")+11);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;
$ee=substr($ee,0,strpos($ee,"'"));$tmp3=substr($tmp2,strpos($tmp2,"'")).$tmp3;$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
 if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);
//echo ($isyoutube?"<br>isyoutube":"<br>notyoutube");
//echo "<br>kk[1]".$kk[1];
...
else if (strpos(strtolower($ee)," data-url=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," data-url=")+10);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);
...
$ee=substr($cc[$dd],0,strpos($cc[$dd],">"));$tmp2=$ee;$tmp3=substr($cc[$dd],strpos($cc[$dd],">"));$tmp1="";
if (strpos(strtolower($ee)," data-src=\"")>0) {$ee=substr($ee,  strpos(strtolower($ee)," data-src=\"")+11);
$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;$ee=substr($ee,0,strpos($ee,"\""));$tmp3=substr($tmp2,strpos($tmp2,"\"")).$tmp3;
$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);
$xurls.=(startsWith($cc[$dd],"s")?(($isyoutube && strpos($kk[1],"/base.js")>0)?"vid\n":"src\n"):"img\n");
...
else if (strpos(strtolower($ee)," data-src='")>0)  {$ee=substr($ee,  strpos(strtolower($ee)," data-src='")+11);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;
$ee=substr($ee,0,strpos($ee,"'"));$tmp3=substr($tmp2,strpos($tmp2,"'")).$tmp3;$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
 if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);
//echo ($isyoutube?"<br>isyoutube":"<br>notyoutube");
//echo "<br>kk[1]".$kk[1];
...
else if (strpos(strtolower($ee)," data-src=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," data-src=")+10);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);
...
else if (strpos(strtolower($ee)," srcset=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," srcset=")+8);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;if (strpos($ee," ")>0)                                        {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;} 
else {

if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
} else {
...
$kk=filelist2($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port,$fcombo,$fileid);$urls=$kk[0];$urls2=$kk[4];$tmp2=$kk[1];$index=$kk[2];$xurls=$kk[3];}
...
else if (strpos(strtolower($ee)," src=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," src=")+5);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;if (strpos($ee," ")>0)                                        {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
 else {
if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
...
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="snd\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=vid";} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=snd";}}
}
...
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="emb\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=vid";} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=emb";}}
}
...
else if (strpos(strtolower($ee)," code=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," code=")+6);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;if (strpos($ee," ")>0)                                        {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;} 
else {
if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
...
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="java\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=java";} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=java";}}
}
...
if (!(endsWith($tmp9,"/videos/\"") || endsWith($tmp9,"/videos/")|| endsWith($tmp9,"/videos/\" ")|| strpos($tmp9,">")>0 )) {
...
$urls2[$kk[3]]=1+count($urls2);
$xurls.="vid\n";$index++;
$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=vid";}
 else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=vid";}}
...
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="vid\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=vid";} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=vid";}}
if ($tmp1.$tmp2.$tmp3===$tmp) {} else {$cc[$dd]=$tmp1.$tmp2.$tmp3;}
$tmp1="";$tmp2="";$tmp3=$cc[$dd];
$ee=substr($cc[$dd],0,strpos($cc[$dd],">"));$tmp2=$ee;$tmp3=substr($cc[$dd],strpos($cc[$dd],">"));$tmp1="";
...
else if (strpos(strtolower($ee)," poster=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," poster=")+8);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;if (strpos($ee," ")>0)                                        {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;} 
else {
if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
...
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="img\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=img";} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=img";}}
}
...
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.=gettype3($kk[1])."\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".gettype3($kk[1]);} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".gettype3($kk[1]);}}
}
...
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
 if ($kk[0]) {
$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.=gettype2($tmp3)."\n";$index++;
$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".gettype2($tmp3);}
else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".gettype2($tmp3);}}

else if (strpos(strtolower($ee)," src='")>0)  {
$ee=substr($ee,  strpos(strtolower($ee)," src='")+6);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee; $ee=substr($ee,0,strpos($ee,"'"));$tmp3=substr($tmp2,strpos($tmp2,"'")).$tmp3;$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.=gettype2($tmp3)."\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".gettype2($tmp3);} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".gettype2($tmp3);}}
...
$ee=substr($ee,0,strlen($ee)-1);
$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}}
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.=gettype2($tmp3)."\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".gettype2($tmp3);} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".gettype2($tmp3);}}
...
else if (strpos(strtolower($ee)," setset=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," srcset=")+8);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;if (strpos($ee," ")>0)                                        {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
 else {
if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
...
$kk=filelist2($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port,$fcombo,$fileid);$urls=$kk[0];$tmp2=$kk[1];$index=$kk[2];$xurls=$kk[3];}
...
else if (strpos(strtolower($ee)," content=")>0)   {$ee=substr($ee,0,strpos(strtolower($ee)," content=")+9);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;if (strpos($ee," ")>0)                                        {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;} 
else {
if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
else {$ee=substr($ee,0,strlen($ee)-1);$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
...
if (strpos(strtolower($cc[$dd])," as=\"style\"")>0) {$casetype="css";}
else if (strpos(strtolower($cc[$dd])," as=style")>0) {$casetype="css";}
else if (strpos(strtolower($cc[$dd])," as='style'")>0) {$casetype="css";}
else {$casetype="js";}}
if (strpos(strtolower($cc[$dd])," rel='preload'")>0) {
if (strpos(strtolower($cc[$dd])," as=\"style\"")>0) {$casetype="css";}
...
if (strpos(strtolower($cc[$dd])," rel=\"dns-prefetch\"")>0) {$casetype="css";}
if (strpos(strtolower($cc[$dd])," rel='dns-prefetch'")>0) {$casetype="css";}
if (strpos(strtolower($cc[$dd])," rel=dns-prefetch")>0) {$casetype="css";}
...
$urls2[$kk[3]]=1+count($urls2);
$xurls.=$casetype."\n";
$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".$casetype;}
 else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".$casetype;}
}
...
$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;
if (strpos($ee," ")>0)                        {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;} 
else {
if (strpos($ee," ")===true) {$ee=substr($ee,1);
if (strpos($ee," ")>0) {$ee=substr($ee,0,strpos($ee," "));$tmp3=substr($tmp2,strpos($tmp2," ")).$tmp3;$tmp2=$ee;}
...
$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.=$casetype."\n";$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".$casetype;}
 else {
//echo "<br>2:".$kk[1];


$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".$casetype;}}
}
...
if ($iscss1 || startsWith(strtolower($cc[$dd]),"style") || strpos($cc[$dd],":url(")<strpos($cc[$dd],">") || strpos($cc[$dd],"json")<strpos($cc[$dd],">") || strpos($cc[$dd],"navigator.serviceWorker.register(")!==FALSE) {
...
$xx=0;
for($xx=0;$xx<count($gg);$xx++) {
if ($xx==0 && !startsWith($cc[$dd],".get(\"")) {$tmp4.=$gg[$xx];}
else {
$h=strpos($gg[$xx],")");
if (strpos($gg[$xx],",")>0 && strpos($gg[$xx],",")<$h) {$h=strpos($gg[$xx],",");}
$ee=substr($gg[$xx],0,$h);
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="src\n";$index++;$tmp4.=".get(\"".$fcombo."?fileid=".$fileid."&index=".$index."&type=img\"";} else {$tmp4.=".get(\"".$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=img\"";}
$tmp4.=substr($gg[$xx],$h);
}//end else xx==0
}//endfor
$cc[$dd]=$tmp4;
...
$xx=0;
for($xx=0;$xx<count($gg);$xx++) {
if ($xx==0 && !startsWith($cc[$dd],"navigator.serviceWorker.register(")) {$tmp4.=$gg[$xx];}
else {
$h=strpos($gg[$xx],")");
if (strpos($gg[$xx],",")>0 && strpos($gg[$xx],",")<$h) {$h=strpos($gg[$xx],",");}
$ee=substr($gg[$xx],0,$h);
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="src\n";$index++;$tmp4.="navigator.serviceWorker.register(\"".$fcombo."?fileid=".$fileid."&index=".$index."&type=src\n";} else {$tmp4.="navigator.serviceWorker.register(\"".$fcombo."?fileid=".$fileid."&index=".$kk[1]."&type=src";}
$tmp4.=substr($gg[$xx],$h);
}//end else xx==0
}//endfor
$cc[$dd]=$tmp4;
...
//$ee=substr($gg[$xx],0,$h);
//$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
//if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="img\n";$index++;$tmp4.="\"src\":\"".$fcombo."?fileid=".$fileid."&index=".$index."&type=img\"";} else {$tmp4.="\"src\":\"".$fcombo."?fileid=".$fileid."index=".$kk[2]."&type=img\"";}
//$tmp4.=substr($gg[$xx],$h);
//}//end else xx==0
//}//endfor
//$cc[$dd]=$tmp4;
...
$ee=substr($gg[$xx],0,strpos($gg[$xx],")"));
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="img\n";$index++;$tmp4.=",url(".$fcombo."?fileid=".$fileid."&index=".$index."&type=img";} 
else {$tmp4.=",url(".$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=img";}
$tmp4.=substr($gg[$xx],strpos($gg[$xx],")"));
}//end else xx==0
}//end startswith data
else {$tmp4.=",url(".$gg[$xx];}
...
$ee=substr($gg[$xx],0,strpos($gg[$xx],")"));
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="img\n";$index++;$tmp4.=",\nurl(".$fcombo."?fileid=".$fileid."&index=".$index."&type=img";}
else {$tmp4.=",url(".$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=img";}
$tmp4.=substr($gg[$xx],strpos($gg[$xx],")"));
}//end else xx==0
}//end startswith data
...
for($xx=0;$xx<count($gg);$xx++) {
if (!(startsWith(strtolower($gg[$xx]),"data:") || startsWith(strtolower($gg[$xx]),"\"data:"))) {
if ($xx==0 && !startsWith($cc[$dd],",uri:")) {$tmp4.=$gg[$xx];}
else {
$ee=substr($gg[$xx],0,strpos($gg[$xx],")"));
$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port);
if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);$xurls.="img\n";$index++;$tmp4.=",uri:".$fcombo."?fileid=".$fileid."&index=".$index."&type=img";} 
else {$tmp4.=",uri:".$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=img";}
$tmp4.=substr($gg[$xx],strpos($gg[$xx],")"));
}}
else {$tmp4.=",url:".$gg[$xx];}
}
...
$li=true;
if (!isset($_SESSION["user"])) {$li=false;}
...
$sql->bind_param("si",$_SESSION["user"],$timen);
...
echo "<table><tr><th>transaction</th><th>url<th>Time</th><th>paid</th>";
    while ($row = $result->fetch_assoc()) {echo "<tr><td><a href=preview1.php?fileid=".$row["fileid"]."&index=0>".$row["fileid"]."</a><br>".(!($row["added"]>0)?" <br><a href=".$finalurl."/".encode($row["fileid"]).">".$finalurl."/".encode($row["fileid"])."</a>":"")."</td><td>".$row["filepath"]."</td><td>".substr(gmdate('r',$row["filetime"]),4);
echo "</td><td>";
echo (floatval($row["m2"])>=floatval($row["payment"]))?"Thank you.".($row["added"]>0?" <br><a href=".$finalurl."/".encode($row["fileid"]).">".$finalurl."/".encode($row["fileid"])."</a>":""):"<a href=".$ss2."/sign/transfer?redirect_uri=".$serverz."%2Fphp%2Fdourl.php&from=".$_SESSION["user"]."&to=".$beneficiary."&amount=".$row["payment"]."STEEM&memo=".$row["fileid"].">Buy Now for ".$row["payment"]." ".$chain."</a></td></tr>";}
$sql->close();
echo "</table>";
...
{echo "<a href='".$ss2."/oauth2/authorize?client_id=".$appname."&redirect_uri=https://blarchive.net/php/dourl.php&scope=vote,comment,custom_json,comment_option'>log in here</a>";}

}
else {{
echo "welcome ".$_SESSION["user"];

if (strlen($_SESSION["user"])==0) {echo "User not logged in<br><a href='".$ss2."/oauth2/authorize?client_id=".$appname."&redirect_uri=https://blarchive.net/php/dourl.php&scope=vote,comment,custom_json,comment_option'>log in here</a>";exit;}
...
{if (startsWith($path,"/p/")) {$ll=template("instagram.com","/p/",templatehelp($url.$path),"","");}
else {$ll=template("instagram.com","",templatehelp($url.$path),"","");}}
...
{
if (startsWith($path,"/pin/")) {$ll=template("pinterest.com","/pin/",templatehelp($url.$path),"","");}
else {
$cpath=$path;
if (startsWith($cpath,"/")) {$cpath=substr($cpath,1);}
if (startsWith($cpath,"/")) {$cpath=substr($cpath,0,strlen($cpath)-1);}
if (substr_count($cpath)==0) {$ll=template("pinterest.com","/user/",templatehelp($url.$path),$url.$path,"");}
else {$ll=template("pinterest.com","",templatehelp($url.$path),"","");}
}}
...
$middle.="<source src=\"".$fcombo."?fileid=".$fileid."&index=8&type=vid\" type=\"video/mp4\"></video>";
...
{
if (strpos(strToLower($path),"?s=")===FALSE) {$istwitter2=true;}
...
$istwitter=true;$istwitter2=false;}
...
//echo "<br>youtube-dl -o ".$serverpath.$fileid."/".$b.".mp4 ".$rr[$b];
...
else if (startsWith($rr[$b],"https://www.youtube.com/watch?v")|| ($rr[$b]==="https://youtu.be/" && strpos($rr[$b],"player")==0 && strpos($rr[$b],"jsbin")==0)) {
...
if (strpos($rr[$b],"ttps://blarchive.net/php/loadfile.php?")>0 && ($isyoutube || $isfacebook)) {
...
$sstr=$fcombo."?fileid=".$fileid."&index=".$b."&type=vid";
...
else if (strpos($rr[$b],"facebook.com")>0 && strpos($rr[$b],"/videos/")>0 && $isfacebook) {
system("youtube-dl  -f bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4 -o ".$serverpath.$fileid."/".$b.".mp4 ".$rr[$b]);
echo "youtube-dl  -f bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4 -o ".$serverpath.$fileid."/".$b.".mp4 ".$rr[$b];
//$sstr=$fcombo."?fileid=".$fileid."&index=".$b."&type=vid";
//$codes[0]=str_replace("https://blarchive.net/php/loadfile.php?fileid=".$fileid."&index=k&type=vid",$sstr,$codes[0]);
...
$result=$sql->get_result();
    while ($row = $result->fetch_assoc()) {
$tick=$row["tickerid"];}}} else {echo "sql failed";}
//echo "<br>tick:".$tick;
if ($tick==="0") {
//$fk=fopen($fileid."/".$c.".mp4",'w');
//fwrite($fk,$codes[$c]);
...
//                  echo ("INSERT INTO `stores` (`fileid`,`fileindex`,`tickerid`,`filedate`,`filename`,`md`) VALUES (".$fileid.",".$c.",?,".time().",?,?)");
...
if(!$sql->execute()){trigger_error("there was an error....".$conn->error, E_USER_WARNING);} else {}
}
