...
$nn=$row["a"];}           
 ...
 else {echo "SQL Error(3): " . $tsql . "" .mysqli_error($conn); }
...
//possibly bug, but if it's not boroque....
if (strpos($ext,".")>0) {$ext=substr($ext,0,strpos($ext,"."));}
...
{
//echo "<br>trimcssimages".$s." ".(startsWith($s,"'")?"true":"false")." ".(endsWith($s,"'")?"true":"false")." ".(startsWith($s," ")?"true":"false");
...
if (strpos($path,"?")>0) {$path=substr($path,0,strpos($path,"?"));echo "path:".$path;}
...
if ($iswp && ($a===$baseurlz."/feed\n" ||$a===$baseurlz."/feed/\n" )) {$a="https://0.0.0.0/adblockedorhashflag\n";echo "<br>culript found";}
else {echo "<br>not culprit:".$a;echo " ".$baseurlz; echo " ".($iswp)?"true":"false";}
...
if ($url==="twitter.com" || $url==="www.twitter.com")  {curl_setopt($ch,CURLOPT_HTTPHEADER, array('accept: */*','Accept-Encoding: identity','User-Agent: Wget/1.19.4 (linux-gnu)'));}
else {curl_setopt($ch,CURLOPT_HTTPHEADER, array('accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,application/signed-exchange;v=b3','accept-language:en-US,en;q=0.9','cache-control: no-cache','DNT:1','pragma:no-cache','sec-fetch-mode: navigate','sec-fetch-site: none','sec-fetch-user: ?1','upgrade-insecure-requests: 1','User-Agent: Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.132 Safari/537.36','Connection:Close'));}
//*/
$result="";
//echo "curl1:"."http".(!startsWith($proto,"s")?"":"s")."://".$url.":".$port.$path;
if(($result=curl_exec($ch)) === false)
{echo '<br>Curl error(1): ' . curl_error($ch);
...
if (startsWith(strtolower($cc[$dd]),"script") || startsWith(strtolower($cc[$dd]),"img ")|| startsWith(strtolower($cc[$dd]),"div ")) {$ee=substr($cc[$dd],0,strpos($cc[$dd],">"));$tmp2=$ee;$tmp3=substr($cc[$dd],strpos($cc[$dd],">"));
if (startsWith(strtolower($cc[$dd]),"script") && !$commentopen) {$scriptopen=true;}


     if (strpos(strtolower($ee)," src=\"")>0) {$ee=substr($ee,  strpos(strtolower($ee)," src=\"")+6);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee;$ee=substr($ee,0,strpos($ee,"\""));$tmp3=substr($tmp2,strpos($tmp2,"\"")).$tmp3;$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);
echo (($isyoutube)?"<br>isyoutube":"<br>notyoutube");
echo "<br>kk[1]".$kk[1];
$xurls.=(startsWith($cc[$dd],"s")?(($isyoutube && strpos($kk[1],"/base.js")>0)?"vid\n":"src\n"):"img\n");
$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".(startsWith($cc[$dd],"s")?"src":"img");} else if (startsWith(strToLower($kk[1]),"data:image")) {$tmp2=$kk[1];} else if (startsWith(strToLower($kk[1]),"\"data:image")) {$tmp2=$kk[1];}  else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".(startsWith($cc[$dd],"s")?"src":"img");}}
else if (strpos(strtolower($ee)," src='")>0)  {$ee=substr($ee,  strpos(strtolower($ee)," src='")+6);$tmp1=substr($cc[$dd],0,strlen($tmp2)-strlen($ee));$tmp2=$ee; $ee=substr($ee,0,strpos($ee,"'"));$tmp3=substr($tmp2,strpos($tmp2,"'")).$tmp3;$tmp2=$ee;$kk=filelist($ee,$urls,$urls2,$xurls,$index,$path,$c_url,$c_proto,$c_port); if ($kk[0]) {$urls.=$kk[1];$urls2[$kk[3]]=1+count($urls2);
//$xurls.=(startsWith($cc[$dd],"s")?"src\n":"img\n");
echo (($isyoutube)?"<br>isyoutube":"<br>notyoutube");
echo "<br>kk[1]".$kk[1];
$xurls.=(startsWith($cc[$dd],"s")?(($isyoutube && strpos($kk[1],"/base.js")>0)?"vid\n":"src\n"):"img\n");
$index++;$tmp2=$fcombo."?fileid=".$fileid."&index=".$index."&type=".(startsWith($cc[$dd],"s")?"src":"img");} else if (startsWith(strToLower($kk[1]),"data:image")) {$tmp2=$kk[1];} else if (startsWith(strToLower($kk[1]),"\"data:image")) {$tmp2=$kk[1];} else {$tmp2=$fcombo."?fileid=".$fileid."&index=".$kk[2]."&type=".(startsWith($cc[$dd],"s")?"src":"img");}}
...
if (false && startsWith(strtolower($cc[$dd]),"video ")) {$ee=substr($cc[$dd],0,strpos($cc[$dd],">"));$tmp2=$ee;$tmp3=substr($cc[$dd],strpos($cc[$dd],">"));
echo "video found";
...
if ($istwitter) {
if (false && startsWith(strtolower($cc[$dd]),"meta  property=\"og:video:secure_url\"")) {$ee=substr($cc[$dd],0,strpos($cc[$dd],">"));$tmp2=$ee;$tmp3=substr($cc[$dd],strpos($cc[$dd],">"));
echo "video found";
...
if ($istwitter) {
if (false && (startsWith($cc[$dd],"div")) && strpos($cc[$dd],"PlayableMedia-player")>0) {$cc[$dd]="div><video width=400 controls><source src=\"".$fcombo."?fileid=".$fileid."&index=".$videocount."&type=vid"."\" type=\"video/mp4\"></video>";$tmp1="";$tmp2="";$tmp3=$cc[$dd];$tmp=$cc[$dd];}}
global $codes;
...
//beginvideo
if (false && strpos($cc[$dd],"PlayableMedia-player")>0) {//startvideo
...
$xx=0;
for($xx=0;$xx<count($gg);$xx++) {
if (!(startsWith(strtolower($gg[$xx]),"data:") || startsWith(strtolower($gg[$xx]),"\"data:"))) {if ($xx==0 && !startsWith($cc[$dd],";:&quot;https:\\/\\/")) {$tmp4.=$gg[$xx];}
else {
...
$sql = $conn->prepare( "select lookup.fileid,filepath,filetime,payment, accounting.fileid, sum(accounting.paid) as m2 from lookup join accounting on lookup.fileid=accounting.fileid WHERE lookup.user=? AND warn=0 AND payment>=0 AND filetime>? group by accounting.fileid order by lookup.fileid DESC");
...
echo "<table><tr><th>transaction</th><th>url<th>Time</th><th>paid</th>";
    while ($row = $result->fetch_assoc()) {echo "<tr><td><a href=preview1.php?fileid=".$row["fileid"]."&index=0>".$row["fileid"]."</a></td><td>".$row["filepath"]."</td><td>".substr(gmdate('r',$row["filetime"]),4);
echo "</td><td>";
echo (floatval($row["m2"])>=floatval($row["payment"]))?"Thank you":"<a href=".$ss2."/sign/transfer?from=".$_SESSION["user"]."&to=".$beneficiary."&amount=".$row["payment"]."STEEM&memo=".$row["fileid"].">Buy Now for ".$row["payment"]." ".$chain."</a></td></tr>";}
$sql->close();
echo "</table>";
...
if ($result=mysqli_query($conn, $tsql)) {
    while ($row = $result->fetch_row()) {       printf ($nn=$row[0]);}
               ...
               echo "SQL Error(1): " . $tsql . "" . mysqli_error($conn);
            }
...
echo "<br>sqlrow:".$fileid."<br>";
               ...
               echo "SQL Error(2): " . $tsql . "" . mysqli_error($conn);
            }
//$xtime=time();
...
if ($rr[$b]."\n"===$video && $istwitter && false) {system("youtube-dl -o ".$serverpath.$fileid."/".$b.".mp4 ".$rr[$b]);
...
else if (false && startsWith($rr[$b],"https://www.youtube.com/watch?v")|| ($rr[$b]==="https://youtu.be/" && strpos($rr[$b],"player")==0 && strpos($rr[$b],"jsbin")==0)) {
...
else {if (false && strpos($rr[$b],"base.js")>0 && $isyoutube) {
